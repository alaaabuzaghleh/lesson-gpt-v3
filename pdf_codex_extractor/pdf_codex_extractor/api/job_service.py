...
from pdf_codex_extractor.codex_runner import CodexError
from pdf_codex_extractor.config import settings
from pdf_codex_extractor.extract import ExtractionManifest
from pdf_codex_extractor.local_vlm_runner import LocalVLMError
from pdf_codex_extractor.opensearch_indexer import OpenSearchIndexer, book_id_for_pdf, build_page_documents
from pdf_codex_extractor.pdf_renderer import PDFRenderer
from pdf_codex_extractor.runner_factory import make_page_runner

from .remote_sync import remote_opensearch_configured, sync_book_documents_from_local
...
PageDocsCallback = Callable[[int, list[dict[str, Any]], int], None]
...
def run_extraction_job(
    *,
    ...
    on_page_indexed: PageDocsCallback | None = None,
    ...
    indexer = OpenSearchIndexer(settings) if index_to_opensearch else None
    ...
    manifest = ExtractionManifest(
        ...
        opensearch_index=settings.opensearch_index if index_to_opensearch else None,
    ...
    with PDFRenderer(pdf_path, pages_dir, dpi=settings.render_dpi) as renderer:
        ...
        for page_number in range(start_page, last_page + 1):
            ...
            if indexer is not None:
                ...
                if on_page_indexed:
                    on_page_indexed(page_number, page_docs, int(success))

                if sync_to_remote and on_page_remote and page_docs:
                ...
                elif sync_to_remote and page_docs and not on_page_remote:
                    from .remote_sync import sync_documents_to_remote

                    remote_success, _remote_errors = sync_documents_to_remote(page_docs)
                    remote_synced += int(remote_success)
            ...
            manifest.indexed_documents = indexed_documents
            manifest_path.write_text(json.dumps(asdict(manifest), ensure_ascii=False, indent=2), encoding="utf-8")
...
def _result_dict(
    *,
    ...
    return {
        ...
        "visual_assets": 0,
        "indexed_records": indexed_documents,
        ...
        "remote_opensearch_index": settings.remote_opensearch_index if settings.remote_opensearch_url else None,
    }
