...
from remote_lessons_gpt.mineru_parser import (
    MinerUBlock,
    MinerUDocument,
    MinerUError,
    MinerUPage,
...
from remote_lessons_gpt.prompts import page_prompt
...
def test_parse_via_http_api_submits_task_and_extracts_zip(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("remote_lessons_gpt.mineru_parser.settings.mineru_api_url", "http://mineru.local")
    monkeypatch.setattr("remote_lessons_gpt.mineru_parser.httpx.Client", FakeClient)
...
def test_pdf_reader_prefers_mineru_text(tmp_path: Path):
    ...
    from remote_lessons_gpt.pdf_reader import PDFReader
...
def test_pipeline_builds_opensearch_ocr_documents(tmp_path: Path):
    ...
    from remote_lessons_gpt.pipeline import BookIngestionPipeline
...
def test_ocr_only_run_checkpoints_each_page(tmp_path: Path, monkeypatch):
    ...
    from remote_lessons_gpt.checkpoint import JobCheckpoint, checkpoint_path
    from remote_lessons_gpt.pipeline import BookIngestionPipeline

    monkeypatch.setattr("remote_lessons_gpt.pipeline.settings.mineru_enabled", False)
    monkeypatch.setattr("remote_lessons_gpt.pipeline.settings.vlm_page_extraction_enabled", False)
...
def test_load_incremental_mineru_document_merges_page_caches(tmp_path: Path):
    from remote_lessons_gpt.mineru_parser import load_incremental_mineru_document
